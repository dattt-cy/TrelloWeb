...
function BoardContent({ board }) {
  ...
  //Function chung xu li viec Cap nhat lai state trong truong hop di chuyen giua cac column khac nhau
  const moveCardBetweenDifferentColum = (
    overColumn,
    overCardId,
    active,
    over,
    ...
    setOrderedColumns((prevColumns) => {
      ...
      if (nextActiveColumn) {
        ...
        // Them placeholder Cảd neu Column rong: Bi keo het cảd di, ko con cai nao nua
      ...
      if (nextOverColumn) {
        ...
        //Phai cap nhat lai chuan du lieu columnId trong cảd sau khi keo card giua 2 column khac nhau
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      ...
      if (oldColumnWhenDraggingCard._id !== overColumn._id) {
        ...
        //Lay vu tri cu cu (tu thang oldColumnWhenDraggingCard)
        ...
        setOrderedColumns((prevColumns) => {
          //CLone mang OrderedColumnsState cu ra mot cai moi de xu li data roi return
          const nextColumns = cloneDeep(prevColumns)
          // Tim toi column ma chung ta dang tha
          const targetColumn = nextColumns.find((c) => c._id === overColumn._id)
          //Cap nhat lai 2 gia tri moi la card va cardOrderIds trong cai target Column
    ...
    //Xu li keo tha column  trong boards content
    ...
    // Nhung du lieu sau khi keo tha luon phai dua ve gia tri
  ...
  // argument: cac doi so tham so
  const collisionDetectionStrategy = useCallback(
    (args) => {
      // Nếu kéo column thì dùng closestCorners
      if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.COLUMN) {
        return closestCorners({ ...args })
      }

      // Tìm các va chạm với con trỏ
      ...
      // Nếu overId là columnId, chỉ trả về columnId, KHÔNG dùng closestCenter nữa
      if (overId) {
        const checkColumn = orderedColumns.find(
          (column) => column._id === overId
        )
        if (checkColumn) {
          // Nếu pointer đang ở giữa column (không trên card nào), chỉ trả về columnId
          lastOverId.current = overId
          return [{ id: overId }]
        }
        // Nếu overId là cardId thì trả về cardId
        lastOverId.current = overId
        return [{ id: overId }]
      }
      // Nếu không có va chạm, trả về lastOverId nếu có
  ...
  return (
    <DndContext
      ...
      //Thuat toan phat hien va cham (neu khong co no thi card voi corner lon se ko keo qua column duoc)
      // Update neu chi dung closestCorner se bi bug giat 2 column
